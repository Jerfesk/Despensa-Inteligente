...
// --- VERIFICAÇÃO E LIMPEZA DA CHAVE ---
//const API_KEY = process.env.API_KEY ? process.env.API_KEY.trim() : undefined;

if (!API_KEY) {
    console.error("❌ ERRO CRÍTICO: O arquivo .env não foi lido ou a chave está vazia!");
} else {
    // Imprime só o comecinho da chave para você ter a prova de que ela foi lida sem vazar tudo
    console.log(`🔑 Chave de API lida do arquivo .env com sucesso. (Inicia com: ${API_KEY.substring(0, 5)}...)`);
}
...
// --- ROTA DE RECEITA (COM DEPURADOR E TRAVA) ---
app.get('/receita/:produto', async (req, res) => {
    // 🛑 TRAVA DE SEGURANÇA: Impede de chamar o Google se não achar a chave
    if (!API_KEY || API_KEY === 'undefined') {
        console.error("Tentativa de gerar receita, mas a API_KEY está undefined.");
        return res.status(500).json({ 
            erro: "O Servidor não conseguiu ler o seu arquivo .env.", 
            detalhe: "O valor da chave está chegando como 'undefined'. Verifique se o arquivo se chama exatamente '.env' e está na mesma pasta do serverScann.js" 
        });
    }
